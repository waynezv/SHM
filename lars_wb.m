...
function [beta, A, mu, C, c, gamma] = lars_wb(X, Y, option, t, standardize)
% X : Phi : 200*500; Y : Xn : 991 * 200
% Least Angle Regression (LAR) algorithm.
...
if standardize,
    ...
    about Y
    about mean_Y   
%     mean_YY = permute(repmat(mean_Y,[size(Y, 1), 1]), []);
 mean_YY = repmat(mean_Y,[size(Y, 1), 1]);
 about mean_YY
...
while nVars < m,
    ...
    about Y
    about mu
    mu_dup = repmat(mu', [size(Y, 1), 1]);
    about mu_dup % 991*200
    c = X'*(Y-mu_dup)';  % Current correlation
    about c % 500*991, otherwise 500*1
