...
%   for its use
%
% File name: SetUpL7_4Flash.m - Example of imaging with single plane wave
%                                 transmit
% Description: 
...
Trans = computeTransBF(Trans);  % L7-4 transducer is 'known' transducer so we can use computeTrans.
...
SeqControl(2).argument = 6000;  % 3 msec
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n).seqControl = [2,nsc]; % time between frames, SeqControl struct defined below.
    ...
    if floor(i/10) == i/10     % Exit to Matlab every 5th frame 
...
save('MatFiles/L7-4Flash'); filename='MatFiles/L7-4Flash';VSX;
return
