...
%              INITIALIZE WEIGHT
            ...
            for i=1:2
                if i==1
                    ...
                    [BW,thresh,gv,gh] = edge(abs(Vn),'sobel',1e-16);
                        for j=2:Q-1
...
%                 save('./data_mat_t/Vn2_s0.1.mat', 'Vn');
            end
            ...
            for k = ClusRed
                ...
                for i = 1:Q
                    if i>min(xx) && i<max(xx)
                         valuePred = polyval(p(k,:),i,[],mu); % use centering and scaling parameter \mu 
                         weight(round(valuePred), i) = penalty2;
                    end
                end
            end
            fprintf('saving weight 2...\n');
%             save('./data_mat_t/W2_s0.1.mat', 'weight');
            
%             % ALTERNATIVE OF ADJUSTING WEIGHT
%             % POLYNOMIAL FIT
%            for k=1:kmean_K
...
%            end
           ...
           % COMPUTE VN AGAIN WITH CLUSTER WEIGHTING
            ...
            fprintf('saving Vn 3...\n');
%             save('./data_mat_t/Vn3_s0.1.mat', 'Vn');
    end
