function [V Phi] = swa_mli2_v2( k, d, X, tau, varargin )
...
%
    ...
    opt.optMatrix     = true;              % Use fully matrix optimization (memory intensive) 
    ...
    switch opt.method
        case 'bp'
            if   opt.optMatrix,
                matlabpool local 12
                   parfor j=1:Q
                     Vn(:,j) = bp(Phi, Xn(j,:).',tau, opt.optNomalized);  
                     j
                   end
%                    Vn = bp(Phi, Xn.', tau, opt.optNomalized);
                fprintf('writing Vn to mat...\n');
                save Vn_bp_1m Vn;
                fprintf('done.');
            else Vn = cell2mat(arrayfun(@(ii) countLoop(@bp, ii, Q, opt.plot*k, Phi, permute(Xn(ii,:,:), [2 1 3]), tau, opt.optNomalized), 1:Q, 'UniformOutput', false )); 
                fprintf('writing Vn to mat...\n');
                save Vn_bp_1m Vn;
                fprintf('done.');
            end
        ...
        case 'mli'
            ...
            for i=1:2
                if i==1
                    ...
                    fprintf('weight after edge detecting...\n');
%                     about weight;
                    fprintf('writing weight to mat...\n');
%                     xlswrite('weight after edge detecting.xls', weight);
                    save weight_1m_li weight;
                    fprintf('done.\n');
                ...
                fprintf('%d ', i);
                fprintf('to mat...\n');
%                 xlswrite('Vn after edge detecting and weighting.xls', Vn);
                save Vn_1m_li Vn;
                fprintf('done.\n');
...
%               feat = zeros(size(orimap));
           ...
           % [U,S,V] = svd(feat);
           % feat2=feat*V;
           save feat_1m_li feat;
           feat2=[feat(:,1) feat(:,2)-feat(:,1)]; % same DIM with feat
%            about feat
%            feat
%            kmean_K=3;
%             [a b]=kmeans(feat2,kmean_K);
            save feat2_1m_li feat2;
           x=feat2'; % DIM 2 * ( i * j )
           [a, model, L] = vbgm(x, 6); % DIM ( i* j ) * 1
...
%                           weight(t2-1:t2+1,i)=weight(t2-1:t2+1,i)*penalty2; 
            ...
            fprintf('weight after clustering and polyfitting...\n');
%             about weight;
            fprintf('writing weight after clustering and polyfitting to mat...\n');
%             xlswrite('weight after clustering and polyfitting.xls', weight);
            save weight_1m_li weight;
            fprintf('done.\n');
            ...
            %wayne in
               ...
               fprintf('writing Vn after clustering and polyfitting to mat...\n');
%                xlswrite('Vn after clustering and polyfitting.xls', Vn);
                save Vn_1m_li Vn;
               fprintf('done.\n');
    ...
    fprintf('writing V Freq-waveno representation to mat...\n');
%     xlswrite('V Freq-waveno representation.xls', V);
    save V_1m_li V;
    fprintf('done.\n');
    % wayne out
    matlabpool close;
