...
Total_Clust_Num = max(label_dev_gt);
Total_Clust_Num
...
tic
dist_mat = cell(K,1);
dist_mat_cosine = cell(K,1);
similarity = cell(K,1);
for k = 1:K
    dist_mat_cosine{k,1} = 0.5.*(1-data_dev2(:,idx_sub{k,1})'*data_dev2(:,idx_sub{k,1}));
...
colum_num_set = round(linspace(10,Total_Clust_Num,10));
...
for k = 1:K
    ...
    score_purity = cluster_purity(label_dev_gt,label_dev_tmp)
    
    %% TD + SVD
    display('TD + SVD');
    tic
    td_svd = max_var_anal2(trans_dist_mat, Total_Clust_Num);
    display(['LCTD-1 SVD takes (Dim Reduction) ' num2str(toc) ' seconds']);
    max_purity = 0;
    for column_num = colum_num_set
        label_dev_tmp = kmeans(td_svd(:,1:column_num), cluster_num{k,1}, 'emptyaction', 'singleton', 'distance', 'cosine');
        score_purity = cluster_purity(label_dev_gt,label_dev_tmp);
        if(score_purity>max_purity)
            max_purity = score_purity;
        end
    end
    max_purity
    ...
    display('LCTD-1');
    tic
    lctd1 = trans_dist_mat.*dist_mat{k,1};
    lctd1 = max_var_anal2(lctd1, Total_Clust_Num);
    display(['LCTD-1 SVD takes ' num2str(toc) ' seconds']);
    max_purity = 0;
    for column_num = colum_num_set
        label_dev_tmp = kmeans(lctd1(:,1:column_num), cluster_num{k,1}, 'emptyaction', 'singleton', 'distance', 'cosine');
        score_purity = cluster_purity(label_dev_gt,label_dev_tmp);
        if(score_purity>max_purity)
            max_purity = score_purity;
        end
    ...
    display('LCTD-2');
    tic
    lctd2 = trans_dist_mat+dist_mat{k,1};
    lctd2 = max_var_anal2(lctd2, Total_Clust_Num);
    display(['LCTD-2 SVD takes ' num2str(toc) ' seconds']);
    max_purity = 0;
    for column_num = colum_num_set
        label_dev_tmp = kmeans(lctd2(:,1:column_num), cluster_num{k,1}, 'emptyaction', 'singleton', 'distance', 'cosine');
        score_purity = cluster_purity(label_dev_gt,label_dev_tmp);
        if(score_purity>max_purity)
            max_purity = score_purity;
        end
    ...
    display('Eu + SVD');
    tic
    eu_svd = max_var_anal2(dist_mat{k,1}, Total_Clust_Num);
    display(['Eu SVD takes ' num2str(toc) ' seconds']);
    max_purity = 0;
    for column_num = colum_num_set
        label_dev_tmp = kmeans(eu_svd(:,1:column_num), cluster_num{k,1}, 'emptyaction', 'singleton', 'distance', 'cosine');
        score_purity = cluster_purity(label_dev_gt,label_dev_tmp);
        if(score_purity>max_purity)
            max_purity = score_purity;
        end
    ...
    display('Cosine + SVD');
    tic
    eu_svd = max_var_anal2(dist_mat_cosine{k,1}, Total_Clust_Num);
    display(['Cosine SVD takes ' num2str(toc) ' seconds']);
    max_purity = 0;
    for column_num = colum_num_set
        label_dev_tmp = kmeans(eu_svd(:,1:column_num), cluster_num{k,1}, 'emptyaction', 'singleton', 'distance', 'cosine');
        score_purity = cluster_purity(label_dev_gt,label_dev_tmp);
    ...
    %% Euclidean
    display('Eu');
    label_dev_tmp = kmeans(dist_mat{k,1}, cluster_num{k,1}, 'emptyaction', 'singleton', 'distance', 'cosine');
    score_purity = cluster_purity(label_dev_gt,label_dev_tmp)
    
    %% Cosine
    display('Cosine');
    label_dev_tmp = kmeans(dist_mat_cosine{k,1}, cluster_num{k,1}, 'emptyaction', 'singleton', 'distance', 'cosine');
    score_purity = cluster_purity(label_dev_gt,label_dev_tmp)

    %% K-means
    display('K-means');
    label_dev_tmp = kmeans(data_dev2', cluster_num{k,1}, 'emptyaction', 'singleton', 'distance', 'cosine');
    score_purity = cluster_purity(label_dev_gt,label_dev_tmp)
