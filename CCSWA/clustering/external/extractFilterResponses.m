...
%   filterResponses:    a W*H x N*3 matrix of filter responses
    ...
    for filterI=0:length(filterBank)-1
        ...
        filterResponses(:,filterI*3+1) = reshape(imfilter(L, filter, 'symmetric'), pixelCount, 1);
        filterResponses(:,filterI*3+2) = reshape(imfilter(a, filter, 'symmetric'), pixelCount, 1);
        filterResponses(:,filterI*3+3) = reshape(imfilter(b, filter, 'symmetric'), pixelCount, 1);
    end
end
