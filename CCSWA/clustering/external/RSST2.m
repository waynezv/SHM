function [label_out] = RSST2(image, label_in, bound_pxl_mask, thre)
...
for i = 1:label_num-1
    for j = i:label_num
        if Adjacency_Matrix(i, j)==1
            label_i = (label_out==i & bound_pxl_mask==1);
            label_j = (label_out==j & bound_pxl_mask==1);
            ...
            dist = sum(sum(pdist_rgb.*pdist_xy))/normalize_weight;
...
while(1)
    ...
    [edge_weight neigh_idx1] = min(dist_set{region_idx, 1});
    ...
    for i = 1:length(common_node)
        ...
        label_i = (label_out==merge_node & bound_pxl_mask==1);
        label_j = (label_out==common_node(i) & bound_pxl_mask==1);
        set_rgb1 = [L(label_i) a(label_i) b(label_i)];
        set_xy1 = [X(label_i) Y(label_i)];
        set_rgb2 = [L(label_j) a(label_j) b(label_j)];
        set_xy2 = [X(label_j) Y(label_j)];
    ...
    for i = 1:length(diff_node1)
        ...
        label_i = (label_out==merge_node & bound_pxl_mask==1);
        label_j = (label_out==diff_node1(i) & bound_pxl_mask==1);
        set_rgb1 = [L(label_i) a(label_i) b(label_i)];
        set_xy1 = [X(label_i) Y(label_i)];
        set_rgb2 = [L(label_j) a(label_j) b(label_j)];
        set_xy2 = [X(label_j) Y(label_j)];
    ...
    for i = 1:length(diff_node2)
        label_i = (label_out==merge_node & bound_pxl_mask==1);
        label_j = (label_out==diff_node2(i) & bound_pxl_mask==1);
        set_rgb1 = [L(label_i) a(label_i) b(label_i)];
        set_xy1 = [X(label_i) Y(label_i)];
        set_rgb2 = [L(label_j) a(label_j) b(label_j)];
        set_xy2 = [X(label_j) Y(label_j)];
