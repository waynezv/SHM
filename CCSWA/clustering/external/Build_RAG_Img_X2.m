function [Graph_Edge] = Build_RAG_Img_X2(label, vertex)
sup_num = max(label(:));
...
%                 display('check!')
%             end
            Graph_Edge(count, 3) = pdist3(vertex(i, :), vertex(neigh_info{i, 1}(1, j), :), 'chisq');%norm(vertex(i, :)-vertex(neigh_info{i, 1}(1, j), :));
