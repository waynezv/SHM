function [label_out] = RSST3(image, label_in, bound_pxl_mask, thre)
...
for i = 1:label_num-1
    for j = i:label_num
        if Adjacency_Matrix(i, j)==1
            ...
            set_lab1 = [L(label_i) a(label_i) b(label_i)];
            set_xy1 = [X(label_i) Y(label_i)];
            set_lab2 = [L(label_j) a(label_j) b(label_j)];
            set_xy2 = [X(label_j) Y(label_j)];
            pdist_lab = pdist2(set_lab1, set_lab2, 'euclidean');
            pdist_xy = pdist2(set_xy1, set_xy2, 'euclidean');
            pdist_xy = exp(-(pdist_xy.^2)./(2*bandwidth^2));
            normalize_weight = sum(pdist_xy(:));
            dist2 = sum(sum(pdist_lab.*pdist_xy))/normalize_weight;
            min_size = min(size_i, size_j);
            %dist = max((dist2-6.5), 0.0001*dist2)*(max((min_size-100), 0.01)/par_k);
