function [DkV Phi] = swa( k, d, X, tau, varargin )
%SWA  Sparse wavenumber analysis
%   [DkV Phi] = SWA( K, D, X, TAU, OPTIONS ) transforms wave data into
...
%
%   OUTPUTS:
%     DkV: Sparse wavenumber solution
...
%
    ...
    Phi = waveframe(d, k);                  % Compute propogation frame
    
    % PERFORM OPTIMIZATION TO COMPUTE SPARSE WAVENUMBER SOLUTION
    switch opt.method
        case 'bp'
            if   opt.optMatrix, DkV = bp(Phi, Xn.', tau, opt.optNomalized);  
            else DkV = cell2mat(arrayfun(@(ii) countLoop(@bp, ii, Q, opt.plot*k, Phi, permute(Xn(ii,:,:), [2 1 3]), tau, opt.optNomalized), 1:Q, 'UniformOutput', false )); end
        case 'omp'
            if   opt.optMatrix, DkV = omp(Phi, Xn.', tau, opt.optNomalized);  
            else DkV = cell2mat(arrayfun(@(ii) countLoop(@omp, ii, Q, opt.plot*k, Phi, permute(Xn(ii,:,:), [2 1 3]), tau, opt.optNomalized), 1:Q, 'UniformOutput', false )); end
    end
