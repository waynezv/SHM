...
%
    ...
    elseif size(d,2) == 2 
        [kx,ky] = meshgrid(k, k);           % Multi-dimensional wavenumber grid
        A = exp(-1j * sqrt( (d(:,1)*kx(:).').^2 + (d(:,2)*ky(:).').^2 ) );  
        D = 1./sqrt( (d(:,1)*kx(:).').^2 + (d(:,2)*ky(:).').^2 );
        Phi = D.*A;                         % Apply geometric attenuation
        rho = 1./sqrt(sum(abs(Phi).^2, 1)); % Normalization
        Phi = bsxfun(@times, Phi, rho);     % Progagation frame     
    else
        error('waveframe:sws:invalidDimensions', 'd can only be M-by-1 or M-by-2.') 
    end
