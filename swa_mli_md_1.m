function [V Phi] = swa_mli_md_1( k, d, X, tau, varargin )
...
% for j=1:Q
%     XTX(:, j) = lars_getXTX(Xn(j, :).');      % this takes long time.
%     Vn(:, j) = lars(Phi, Xn(j, :).', 'lasso', XTX(:, j), stopCriterion);
% end
% for j=1:Q
%    [beta(:, j),~] = lars(Phi, Xn(j, :)', 'lasso');
% end
%% this works
% nMeas = size(Xn, 2);
% % nMeas
...
% for count = 1 : Q
% %     for idx = 1 : nMeas
%         [Vn_h,~] = lars(Phi, Xn(count, :), 'lasso');
% %     [Vn_h,~] = lars(Phi(:, idx), Xn, 'lasso');
% %     end
%     about Vn_h % 1*500
%     Vn(:, count) = Vn_h;
%     about Vn
...
%     stopCriterion{1,1} = 'maxKernels';
%     stopCriterion{1,2} = 100;
%     % not sure put XTX inside for-loop or not
%     
%     for count = 1 : Q
%         XTX = lars_getXTX(Xn(count, :));
%         sol = lars_l(Xn(count, :), Phi, 'lasso', XTX, stopCriterion);
%         
%     about sol% 1*500
%     Vn(:, count) = struct2cell(cell2mat(sol));
%     
% %     Vn = sol()
%     about Vn
% %     Q
...
%    about Vn
    ...
    % 此前 Vn 经过一次边缘检测增加outlier权重去掉outlier得到   
    % 以下 code does not make sense
    % 把Vn大于0.02的值赋予feat
    % Vn 为complex，转为real
%     Vn = sqrt(real(Vn).^2 + imag(Vn).^2);
            for ii=1:1
%                 orimap=abs(Vn)>0.02;% 此处取了绝对值 可否用complex？
                orimap=abs(Vn)>mean(mean(Vn));
%                 save orimap2 orimap % 500*991
                feat=[];
...
%                 save feat2 feat;% 1713*2
    ...
    % Vn --> feat --> weight
    % 此处Vn经过上面变换 接下来进行聚类 然后根据聚类结果polyfit
               % 聚类得到cluster数kmean_K
               x=feat2';
%                save x_feat2 x;
%                x = feat;
