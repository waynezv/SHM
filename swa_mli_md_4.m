function [V Phi] = swa_mli_md_4( k, d, X, tau, varargin )
% 直接聚类 调整penalty
...
%     about Phi 
    ...
    switch opt.method
       ...
       load Vn_unweighted_md3;
                ...
                orimap=abs(Vn)>mean(mean(abs(Vn)));% 此处取了绝对值 可否用complex？
%                 save orimap orimap;
                feat = [];
...
%    save feat2ij feat2; % 1826*2
%                [U,S,V] = svd(feat2);
%                feat2=feat*V;
               x=feat2';
               [a, model, L] = vbgm_wz_1(Vn, 10);
               label_name=unique(a);
